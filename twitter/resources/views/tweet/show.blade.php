                ...
                @if($tweet->id === $reply->post_id)
                    <div class="card mb-2 rounded-pill">
                        <div class="card-body p-3 w-100">
                            ...
                            <div class="d-flex justify-content-end mx-3 my-1">
                            <button type="button" class="btn btn-danger ms-2" data-bs-toggle="modal" data-bs-target="#deleteReply{{ $reply->id }}">
                                    削除
                            </button>
                                <!-- Modal -->
                            <div class="modal fade" id="deleteReply{{ $reply->id }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header d-block text-center">
                                            <h5 class="modal-title" id="staticBackdropLabel">リプライ削除</h5>
                                        </div>
                                        <div class="modal-body text-center">
                                            本当に削除してもいいですか？
                                        </div>
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                                            @if ($reply->user_id == Auth::id())
                                            <form class="delete-form" action="{{ route('tweet.deleteReply', ['id' => $reply->id]) }}" method="post">
                                                @method('delete')
                                                @csrf
                                                <button type="submit" class="btn btn-danger">削除</button>
                                            </form>
